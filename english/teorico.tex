...
\begin{align}
	...
	(k_z^i + k_z^j) \left( {\textbf{e}_i^\perp}^* \cdot \textbf{e}_j^\perp  \right)  + i \left( \textbf{e}_j^\perp \cdot \nabla_\perp e_{z,i}^* - {\textbf{e}_i^\perp}^* \cdot \nabla_\perp e_{z,j} \right) \Big] \ dxdy \ , \label{eqn:pij-simpli} \\
...
Using the equations for the transverse components (\ref{eqn:transversal}) in the case of fundamental TE modes (\ref{eqn:TEanti}), we have:
\begin{align*}
