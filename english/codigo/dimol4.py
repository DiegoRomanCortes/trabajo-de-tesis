...
dn1 = 30.0E-4  # 4.1E-4 Amplitude of the refractive index modulation
wavelength = 730E-9  # Wavelength of the light
k0 = 2 * np.pi / wavelength  # Wavenumber

distances = np.linspace(5e-6, 20e-6, 16)

# print(f"Distances: {distances*1e6:.2f} um")

np.save("dimol4/distances.npy", distances)
...
for idx, dist in enumerate(distances):
    ...
    np.save(f"dimol4/eigenvalues_{idx}.npy", eigenvalues)
    np.save(f"dimol4/eigenvectors_{idx}.npy", eigenvectors)