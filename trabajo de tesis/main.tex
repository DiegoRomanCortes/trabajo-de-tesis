...
Ecuaciones de Maxwell para un medio lineal no magnético libre de fuentes de carga y de corriente.
\begin{align}
...
\end{align}
donde \textbf{E} y \textbf{B} son los campos eléctrico y magnético respectivamente. La técnica de fabricación por láser pulsado de femtosegundos permite modificar el índice de refracción nominal del borosilicato $n_0 = 1.48$ en $\Delta n \sim 10^{-4}-10^{-3}$, por lo que en general el índice de refracción dependerá de las coordenadas transversales al eje óptico, es decir, $n \equiv n(x,y) = n_0 + \Delta n(x,y)$.

Aplicando rotor por la izquierda a la ecuación (\ref{eqn:faraday-lenz}), usando la ecuación (\ref{eqn:ampere-maxwell}) y asumiendo una solución temporal harmónica en $\omega$ se tiene:

\begin{align}
	\nabla\times\nabla\times\textbf{E} &= -\frac{\partial}{\partial t}(\nabla\times\textbf{B}) = -\frac{n^2}{c^2}\frac{\partial^2 \textbf{E}}{\partial t^2} = n^2k_0^2 \textbf{E},
\end{align}
