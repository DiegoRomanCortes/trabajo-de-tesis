...
ax2.plot(L/(Ns.get()-1), speeds, '.-', color='gray', label=r'tiempo')

ax.set_xlabel(r'$\Delta x$ ($\mu$m)')
ax.set_ylabel(r'$\Delta n_{\text{eff}}/n_{\text{eff}}^\text{analitico}$')
ax.set_xscale('log')
ax.set_yscale('log')
ax.plot(L/(Ns.get()-1), delta_ks.get()[:, 0]/k0, '*', label=r'Modo $1$')
ax.plot(L/(Ns.get()-1), delta_ks.get()[:, 1]/k0, '*', label=r'Modo $2$')
ax.plot(L/(Ns.get()-1), delta_ks.get()[:, 2]/k0, '*', label=r'Modo $3$')
ax.plot(L/(Ns.get()-1), delta_ks.get()[:, 3]/k0, '*', label=r'Modo $4$')
