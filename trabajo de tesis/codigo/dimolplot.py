...
# Load single data
eigenvalues1 = np.load('eigenvalues1.npy')
eigenvalues2 = np.load('eigenvalues2.npy')
...
E1 = np.abs(np.load('eigenvectors1.npy')[:, -1])
E2 = np.abs(np.load('eigenvectors2.npy')[:, -1])
...
for idx, dist in enumerate(distances):
    eigenvalues_couplers[idx, :] = np.load(f'eigenvalues_{idx}.npy')
    dnmdn1s[idx, :] = np.load(f'dn1_array_{idx}.npy')
    dnmdn2s[idx, :] = np.load(f'dn2_array_{idx}.npy')
...
fig.savefig('coupler.png', dpi=500)
plt.close("all")
...
for idx, dist in enumerate(distances):
    ...
    fig, ax = plt.subplots(1, 1)
    E1 = np.load(f'eigenvectors_{idx}.npy')[:, -1]

    E2 = np.load(f'eigenvectors_{idx}.npy')[:, -2]
    ...
    fig.savefig(f'eigenvectors_{idx}.png', dpi=500)
