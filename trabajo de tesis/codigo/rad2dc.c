...
int main(int argc, char* argv[]){    
    ...
    double l0 = atof(argv[1])*1E-9; // wavelenght of light
    double wx = 2.2E-6; // width of the waveguide 
    double wy = 3.0E-6; // height of the waveguide
    double sigma = 7.0E-6; // width of LG-mode
    ...
    for(i = 0; i < Nx; i++){
        for(j = 0; j < Ny; j++){
            ...
            for(int n=-9; n<10; n++){
                for(int m=-9; m<10; m++){
                    dn[i+Nx*j] += dn1 * tanh(33.0 / (exp(((xi-n*d1x)/wx)*((xi-n*d1x)/wx) + ((yj-m*d1y)/wy)*((yj-m*d1y)/wy))));
                }
            }
            printf("%e\n", dn[i+Nx*j]);
        }
