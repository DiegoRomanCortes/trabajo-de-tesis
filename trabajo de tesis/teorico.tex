...
\begin{align}
	\textbf{e}_{j\perp} &= \hat{\textbf{y}}\frac{i\omega\mu_0 H_{a1}}{k_0^2n^2 - k_z^2}\left\{ \begin{matrix}
-\alpha \cos(\alpha x), |x|\le a
\\
\beta\frac{|x|}{x}\sin(\alpha a) e^{-\beta(|x|-a)}, |x| > a
\end{matrix}\right.
\\
\textbf{h}_{j\perp} &= \hat{\textbf{x}}\frac{ik_z^j H_{a1}}{k_0^2n^2 - k_z^2}\left\{ \begin{matrix}
\alpha \cos(\alpha x), |x|\le a
\\
-\beta\frac{|x|}{x}\sin(\alpha a) e^{-\beta(|x|-a)}, |x| > a
\end{matrix}\right.
\end{align}
$x_j=jb$, con $b\ge a$ y $L=\int dy$. Elementos diagonales tienen la forma:
\begin{align*}
	\left(\textbf{e}_j\times\textbf{h}_j^*\right)\cdot \hat{\textbf{z}}&=\frac{k_z^j\omega\mu_0 H_{a1}^2}{(k_0^2n^2-k_z^2)^2} \left\{ \begin{matrix}
...
\begin{align*}
 ...
 \right]
 \\ 
 &= \frac{k_z\omega\mu_0H_{a1}^2 L a}{2 \alpha^2} \left[1 + \frac{\sin^2(\alpha a)}{a \beta^3 } k_0^2 \left( n_1^2 - n_0^2 \right) \right]
\end{align*}
\begin{align*}
