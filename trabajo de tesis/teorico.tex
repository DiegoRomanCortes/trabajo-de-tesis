...
En este caso, las condiciones de corte se dan cuando los términos $\frac{J'_{0}(\alpha a)}{\alpha a J_0(\alpha a)}$ y $\frac{K'_0(\beta a)}{\beta a K_0(\beta a)}$ divergen simultáneamente. Esto ocurre cuando $\alpha a = V$ y $J_0(\alpha a) = 0$. Denotando $x_{0,m}=$2.405,  5.520,  8.654, $\cdots$ al $m$-ésimo cero de la funcion $J_0(x)$, la longitud de onda de corte está dada por 
\begin{equation}
...
El caso $\ell=1$ es de especial interés. La $m$-ésima asíntota estará determinada por el $m$-ésimo cero de $J_1(x)$. Por lo que, debido a un razonamiento similar al caso $\ell =0$, las condiciones de corte son
\begin{align*}
	(\lambda_c)_{1,m} &= 2\pi a \frac{\sqrt{n_1^2 - n_0^2}}{x_{1,m}} \quad \text{(modos HE)}_{1m}
	\\
	(\lambda_c)_{1,m} &= 2\pi a \frac{\sqrt{n_1^2 - n_0^2}}{x_{1,m+1}} \quad \text{(modos EH)}_{1m}
\end{align*}
con $x_{1m}=0$, 3.832,  7.016, 10.173, $\dots$.
...
\begin{align*}
	...
	\\
	 \sum_{\nu, \mu, m, m'}\bar{v}^{*(i)}_{\mu, m'} C_{\nu, \mu, m, m'}u^{(j)}_{\nu, m}  &= \lambda_j \delta_{i, j} +\sum_{\nu, \mu, m, m'}\bar{v}^{*(j)}_{\mu, m'} D_{\mu, \nu, m', m} u^{(j)}_{\nu, m}
	 \\
	 &= \lambda_i^* \delta_{i, j}
\end{align*}
...
\end{align*}
Considerando el par de transformaciones $a_{\mu, m'}\to a'_{\mu, m'} = e^{i\phi}a_{\mu, m'}$, $\bar{b}^*_{\mu, m'}\to \bar{b'}^*_{\mu, m'} = e^{-i\phi}\bar{b}^*_{\mu, m'}$ que dejan invariante el Lagrangiano, considerando $\phi \ll 1$ se tiene $\delta a_{\mu, m'} = i\phi a_{\mu, m'}$ y $\delta \bar{b}^*_{\mu, m'} = -i\phi \bar{b}^*_{\mu, m'}$, por lo que la cantidad conservada, $P$, es:
\begin{equation}
