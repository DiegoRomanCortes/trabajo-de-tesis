...
\begin{equation*}
 D_{\mu, \nu, m', m} = -D_{\nu, \mu, m, m'} = \frac{\epsilon_0 c}{4k_0}\int\displaylimits_{-\infty}^{+\infty}\int\displaylimits_{-\infty}^{+\infty} (n^2_{m'}- n^2_{m}) \textbf{E}_{\mu, m'} \cdot \textbf{E}_{\nu, m}^*   dxdy
\end{equation*}
...
\end{equation}
Redefiniendo $C_{\mu, \mu, m', m,'} \equiv \beta_{\mu, m'}$, es posible encontrar un Lagrangiano para la ecuación (\ref{eqn:CMT1}) definiendo nuevas variables $\bar{v}_{\mu, m'}$ que en sistemas no Hermíticos se conocen como vectores izquierda \cite{NHHamiltonian}:
\begin{equation}
	L = \sum_{\mu, m'} \bar{v}^*_{\mu, m'}\left( i \frac{ d u_{\mu, m'}}{dz}   -
\sum_{\nu, m} C_{\mu, \nu, m', m}  u_{\nu, m}\right)
\end{equation}
Notando que el momentum generalizado es $\Pi_{\mu, m'} \equiv \frac{\partial L}{\partial \dot{u}_{\mu, m'}} = i\bar{v}^*_{\mu, m'}$ el Hamiltoniano del sistema se escribe como:
\begin{align*}
	H &= \sum_{\mu, m'} \Pi_{\mu, m'} \dot{{u}}_{\mu, m'} - L = \sum_{\nu, \mu, m, m'} C_{\mu, \nu, m', m} u_{\nu, m}\bar{v}^*_{\mu, m'}
\end{align*}
Variando el Lagrangiano $L$ se tiene \cite{NHsymmetries}:
\begin{align*}
	\delta L &= \sum_{\mu, m'} \frac{\partial L}{\partial u_{\mu, m'}} \delta u_{\mu, m'} +  \frac{\partial L}{\partial \bar{v}^*_{\mu, m'}} \delta \bar{v}^*_{\mu, m'} + \ \frac{\partial L}{\partial \dot{u}_{\mu, m'}} \delta \dot{u}_{\mu, m'} + \frac{\partial L}{\partial \dot{\bar{v}}^*_{\mu, m'}} \delta \dot{\bar{v}}^*_{\mu, m'}
	\\
	&= \sum_{\mu, m'} \left( \frac{\partial L}{\partial u_{\mu, m'}} - \frac{d}{dz}\frac{\partial L}{\partial \dot{u}_{\mu, m'}} \right)\delta u_{\mu, m'} + \left( \frac{\partial L}{\partial\bar{v}^*_{\mu, m'}} - \frac{d}{dz}\frac{\partial L}{\partial  \dot{\bar{v}}^*_{\mu, m'}} \right)\delta \bar{v}^*_{\mu, m'} + \frac{d}{dz}\left(\frac{\partial L}{\partial \dot{u}_{\mu, m'}}\delta u_{\mu, m'} +  \frac{\partial L}{\partial \dot{\bar{v}}^*_{\mu, m'}}\delta \bar{v}^*_{\mu, m'}\right)
	\\	
	&\implies  \frac{d}{dz} \sum_{\mu, m'}\left(\frac{\partial L}{\partial \dot{u}_{\mu, m'}}\delta u_{\mu, m'} +  \frac{\partial L}{\partial \dot{\bar{v}}^*_{\mu, m'}}\delta \bar{v}^*_{\mu, m'}\right) = 0.
\end{align*}
Considerando el par de transformaciones $u_{\mu, m'}\to u'_{\mu, m'} = e^{i\phi}u_{\mu, m'}$, $\bar{v}^*_{\mu, m'}\to \bar{v'}^*_{\mu, m'} = e^{-i\phi}\bar{v}^*_{\mu, m'}$ que dejan invariante el Lagrangiano, infinitesimalmente se tiene $\delta u_{\mu, m'} = i\phi u_{\mu, m'}$ y $\delta \bar{v}^*_{\mu, m'} = -i\phi \bar{v}^*_{\mu, m'}$, por lo que la cantidad conservada $P$ es:
\begin{equation}
	P = \sum_{\mu, m'} \bar{v}^*_{\mu, m'} u_{\mu, m'}.
\end{equation}
