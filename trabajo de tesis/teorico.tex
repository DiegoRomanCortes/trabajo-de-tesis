...
\end{align}
Claramente $P_{ij}$ es una matriz hermítica. A primera vista, puede parecer que $H_{ij}$ no es hermítica, pero veamos que sí lo es. Para ello, concentremonos en los términos extremos
\begin{align*}
H_{ji}^* - H_{ij}= P_{ji}^* k_z^i - P_{ij} k_z^j + \frac{\omega \varepsilon_0}{4} \iint(n_j^2-n_i^2) \textbf{e}_i^* \cdot \textbf{e}_j dxdy
	\end{align*}
