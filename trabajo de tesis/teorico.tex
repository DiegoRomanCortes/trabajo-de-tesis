...
\subsection{Desde un principio variacional}

A partir de la ecuaciones (\ref{eqn:Efield}) y (\ref{eqn:Hfield}) es posible depejar la constante de propagación $k_z$ en términos del vector de Poynting $\textbf{S} = \frac{1}{2} \text{Re}\{\textbf{E} \times \textbf{H}^*\}$ 
...
\end{equation}
$\textbf{E} = \sum_i a_i \textbf{e}_i$, $\textbf{H} = \sum_i a_i \textbf{h}_i$
\begin{align*}
...
\end{align}
Se puede escribir la expresión (\ref{eqn:kzprevar}) para la constante de propagación $k_z$ de manera compacta:
\begin{equation}
	k_z = \frac{\sum_{ij} a_i^* H_{ij} a_j}{\sum_{ij} a_i^*P_{ij} a_j}
\end{equation}
\begin{align}
	k_z \sum_{j} P_{ij} a_j &= \sum_{j} H_{ij} a_j 
	\\
	-i \sum_{j} P_{ij} \frac{d a_j}{dz} &= \sum_{j} H_{ij} a_j
\end{align}
Claramente $P_{ij}$ es una matriz hermítica. A primera vista, puede parecer que $H_{ij}$ no es hermítica, pero veamos que sí lo es. Para ello, se hace necesario considerar la resta $H_{ij} - H_{ji}^*$ y notar que se anula:
\begin{align*}
...
0,
	\end{align*}
donde se ha usado que los campos deben decaer a cero en el infinito. 
\begin{equation}
	L = \sum_{ij}  a_i^*\left(i  P_{ij} \frac{d a_j}{dz} + H_{ij} a_j \right).
\end{equation}
El momentum generalizado de este Lagrangiano es $\Pi_k=\frac{\partial L}{\partial \dot{a_k}} = i\sum_{j} a_j^* P_{jk}$, por lo que el Hamiltoniano asociado es
\begin{equation}
...
\begin{equation}
	P = \sum_{ij} a_i^* P_{ij} a_j.  \label{eqn:power}
\end{equation}
...
Al considerar el par de transformaciones asociadas al grupo unitario U(1), $a_{\mu, m'}\to a'_{\mu, m'} = e^{i\phi}a_{\mu, m'}$, ${a}_{\mu, m'}^* \to {a'}_{\mu, m'}^* = e^{-i\phi}{a}_{\mu, m'}^*$ que dejan invariante el Lagrangiano, y si se toma una variación infinitesimal $\phi \ll 1$ se tiene $\delta a_{\mu, m'} = i\phi a_{\mu, m'}$ y $\delta {a}_{\mu, m'}^* = -i\phi {a}_{\mu, m'}^*$, por lo que la cantidad conservada, $P$, que se identifica con la potencia total del sistema es:
\begin{equation}
	P = \sum_{\mu, m'} |a_{\mu, m'}|^2 = \langle a|a\rangle. \label{eqn:power}
\end{equation}
Se tienen entonces dos cantidades conservadas en la dinámica, $H$ y $P$, que serán de utilidad para verificar la validez de soluciones numéricas a la ecuación (\ref{eqn:CMT_mat}).

Las mismas ecuaciones se pueden obtener de dos formas más: a través de un principio variacional \citep{haus_coupled-mode} y desde un dímero hacia $n$ guías \citep{christodoulides_discrete_1988}.
