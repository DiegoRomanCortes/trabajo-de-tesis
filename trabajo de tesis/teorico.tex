...
El célebre modelo de Su-Schrieffer-Heeger (SSH) es uno de los sistemas más sencillos que exhibe \textit{estados de borde topológicos} \cite{ssh, ssh-photonic, topological-photonics}. La red SSH fotónica se usará como ejemplo de las herramientas analíticas que posteriormente se utilizarán.

\begin{figure}[H]
\centering
	\includegraphics[width=0.9\linewidth]{media/ssh-model.png}
	\caption{Esquema del modelo SSH. En amarillo se encierra la celda unitaria, donde se distinguen los sitios A y B y el acoplamiento intracelda (intercelda) $t$ ($t'$).}
\end{figure}
\begin{equation}
