...
\begin{equation*}
	...
	\left\{	
	\begin{matrix}
		A\left[\boldsymbol{\hat{\rho}}\alpha J'_\ell (\alpha \rho)\cos(\ell \phi) - \boldsymbol{\hat{\phi}} \ell J_\ell (\alpha \rho)\sin(\ell \phi)/\rho\right] e^{i k_z z}, \quad \text{si } \rho \le a  
		\\
		B\left[\boldsymbol{\hat{\rho}}\beta K'_\ell (\beta\rho) \cos(\ell \phi) - \boldsymbol{\hat{\phi}}\ell K_\ell (\alpha \rho)\sin(\ell \phi)/\rho \right] e^{i k_z z} , \quad \text{si } \rho > a  
	\end{matrix}
	\right.
\end{equation*}
...
\begin{align*}
		H_z &= H_0 e^{i k_z z}
	  	 \left\{
		\begin{matrix}	  	 
	  	 J_\ell (\alpha \rho)\cos(\ell \phi), \quad \text{si } \rho \le a  
	  	 \\
	  	 K_\ell (\beta \rho)\cos(\ell \phi), \quad \text{si } \rho > a  
	  	 \end{matrix}
	  	 \right.	
		\\
...
\begin{align}
	...
	\label{eqn:CMT1}
\end{align}
Las ecuciones (\ref{eqn:CMT1}) se conocen como ecuaciones discretas tipo Schrödinger, donde se han definido y usado
\begin{align*}
