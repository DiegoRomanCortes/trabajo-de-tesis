...
Esta tesis estudia el comportamiento de luz láser de baja potencia (1 mW de potencia de salida) propagada en guías de onda escritas dentro de una muestra de vidrio borosilicato.  
Es por ello que se supone un medio lineal no magnético sin fuentes de carga ni de corriente libres. 
...
Aplicando rotor por la izquierda a la ecuación de Faraday-Lenz (\ref{eqn:faraday-lenz}), usando la ecuación de Ampère-Maxwell (\ref{eqn:ampere-maxwell}) y asumiendo una solución temporal armónica proporcional a $e^{-i\omega t}$ se tiene:
\begin{align}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\linewidth]{media/coupling.pdf}
...
Es posible plantear un principio de mínima acción si se propone el siguiente Lagrangiano tipo Schrödinger para la ecuación (\ref{eqn:CMT1}):
\begin{equation}
...
\begin{align}
	H &= \sum_{\mu, m'} \Pi_{\mu, m'} \dot{{a}}_{\mu, m'} - L = i\sum_{\nu, \mu, m, m'} C_{\mu, \nu, m', m} \Pi_{\mu, m'} a_{\nu, m} = \sum_{\nu, \mu, m, m'} C_{\mu, \nu, m', m} |a_{\nu, m}|^2 = \langle a|\hat{C}| a \rangle  \label{eqn:hamiltoniano}
\end{align}
