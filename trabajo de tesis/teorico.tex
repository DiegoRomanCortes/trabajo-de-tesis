...
\end{align}
donde se ha usado la aproximación de guiaje débil para anular el lado de derecho de la ecuación (\ref{eqn:helmholz}), aunque la ecuación es exacta para la componente longitudinal del campo eléctrico $E_z$. Notemos que la ecuación (\ref{eqn:eigenfield}) es un problema de autovalores $\beta_\nu^2$ y autofunciones $\textbf{E}_\nu(x,y)$, las cuales son ortogonales (ver apéndice \ref{sec:orto}). En principio, la forma espacial del índice de refracción $n(x, y)$ puede ser arbitraria siempre y cuando que se satisfaga la condición de guiaje débil. 
...
\begin{align*}
	P_{ij} &= k_z^i L \omega \mu_0 H_{a1}^2 \beta\sin^2(\alpha a)e^{-\beta b} e^{2\beta a} \left[\frac{1}{\alpha^2(\alpha^2+\beta^2)} + \frac{e^{-2\beta a}+\beta\left(b-2a\right)}{2\beta^4} \right]			
\end{align*}

\begin{align*}
	\tilde{H}_{ij} &\equiv H_{ij} - P_{ij} k_z^j = \frac{\omega\epsilon_0}{4} \iint (n^2 - n_j^2)\textbf{e}_j\cdot \textbf{e}_i^* dx dy 
\\	
	&= \frac{k_0 \mu_0 L}{2} \frac{ H_{a1}^2}{\alpha^2} \beta \cos^2(\alpha a)e^{2\beta a} e^{-\beta b} 
\end{align*}
...
\begin{align*}
	P_{jj} &= \frac{k_z^j L \omega\mu_0H_{a1}^2  a}{2 \alpha^2} \left[1 + \frac{\sin^2(\alpha a)}{a \beta^3 } k_0^2 \left( n_1^2 - n_0^2 \right) \right]
\end{align*}
\begin{align*}
	\tilde{H}_{jj} &\equiv H_{jj} - P_{jj} k_z^j = \frac{\omega\epsilon_0}{4} \iint (n^2 - n_j^2)|\textbf{e}_j|^2 dx dy
	\\
	&=
	\frac{\omega \varepsilon_0 L \sin^2(\alpha a)e^{\beta a}e^{-2\beta b}}{4 \beta^3} (n_1^2-n_0^2) \sinh(2\beta a)
\end{align*}

La dinámica está regida por el sistema acoplado de las ecuaciones (\ref{eqn:non-ortho-CMT-eqs}), que luego de premultiplicar por la inversa de la matriz $P_{ij}$:
\begin{align*}
	...
	\end{pmatrix}
	\begin{pmatrix}
	P_{ii}k_z^j + \tilde{H}_{ii} & P_{ij} k_z^j + \tilde{H}_{ij}
	\\
	P_{ij} k_z^j + \tilde{H}_{ij} & P_{ii} k_z^j + \tilde{H}_{ii}
	\end{pmatrix}
	\begin{pmatrix}
	a_1
	\\
	...
	&= 	
	\begin{pmatrix}
	k_z^j + \frac{P_{ii} \tilde{H}_{ii} -P_{ij}\tilde{H}_{ij}}{P_{ii}^2-P_{ij}^2} & \frac{P_{ii} \tilde{H}_{ij}  -P_{ij}\tilde{H}_{ii}}{P_{ii}^2-P_{ij}^2}
	\\
	\frac{P_{ii} \tilde{H}_{ij} - P_{ij}\tilde{H}_{ii}}{P_{ii}^2-P_{ij}^2} &  k_z^j +\frac{P_{ii} \tilde{H}_{ii}  -P_{ij}\tilde{H}_{ij}}{P_{ii}^2-P_{ij}^2}
	\end{pmatrix}
	\begin{pmatrix}
	a_1
	\\
