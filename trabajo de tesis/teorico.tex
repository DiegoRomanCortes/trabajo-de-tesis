...
\begin{align*}
	...
	\end{pmatrix} &= 	
	\begin{pmatrix}
	k_z^{\text{TE}} + \delta k & V
	\\
	V &  k_z^{\text{TE}} +\delta k
	\end{pmatrix}
	\begin{pmatrix}
	a_1
	\\
...
\begin{align*}
	...
	\end{pmatrix}
	=
	\exp\left\{{i\left[(k_z^{\text{TE}}+\delta k)I + V\sigma_x\right]z }\right\}	\begin{pmatrix}
	a_1
	\\
	a_2
	\end{pmatrix}
	=
	e^{i(k_z^{\text{TE}}+\delta k)z}\frac{1}{\sqrt{2}}
	\begin{pmatrix}
	1 & 1
	\\
	1 & -1
...
\cite{ssh}
\begin{equation}
	H = \sum_n C_\text{intra} b_n^*a_n + C_\text{inter} \left(b_{n-1}^*a_n + a_{n+1}^*b_n \right) + c.c.
\end{equation}
