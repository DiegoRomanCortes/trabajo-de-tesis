...
\end{align*}
Este sistema presenta simetría global U(1), pues la transformación $u_{\mu, m'} \to u'_{\mu, m'}= u_{\mu, m'}e^{i\phi}$ deja invariante el Lagrangiano y el Hamiltoniano $\forall \phi$. Expandiendo, se tiene $u'_{\mu, m'} \approx u_{\mu, m'}(1+i\phi)$. Notemos que escribiendo el Lagrangiano respecto a estas nuevas variables, despreciando términos proporcionales a $\phi^2$ y usando las ecuaciones de movimiento se tiene:
\begin{align*}
	L' &= L + \frac{i}{2}\sum_{\mu, m'}\left(\phi i\frac{d\bar{u}_{\mu, m'}}{dz}u_{\mu, m'} -i\frac{d(\phi \bar{u}_{\mu, m'})}{dz}u_{\mu, m'} - i\frac{d (\phi u_{\mu, m'})}{dz} {\bar{u}}_{\mu, m'}+\phi i\frac{d u_{\mu, m'}}{dz} {\bar{u}}_{\mu, m'}\right)
	\\
	&=
	L + \frac{1}{2}\sum_{\mu, m'}\left(\frac{d(\phi \bar{u}_{\mu, m'})}{dz}u_{\mu, m'} +\frac{d (\phi u_{\mu, m'})}{dz} {\bar{u}}_{\mu, m'}\right) = L + \frac{1}{2}\sum_{\mu, m'}\left(\phi \frac{d |u_{\mu, m'}|^2}{dz} + |u_{\mu, m'}|^2\frac{d\phi}{dz}\right) 
	\\
	&= L + \frac{d}{dz} \left(\frac{\phi}{2} \sum_{\mu, m'} |u_{\mu, m'}|^2 \right)
\end{align*}
En vitud del Teorema de Noether, hemos encontrado una cantidad conservada a partir de una derivada total inducida por una transformación infinitesimal de parámetro $\phi$. Esta cantidad $P$ es proporcional a la potencia total del sistema:
\begin{equation*}
	P =\sum_{\mu, m'} |u_{\mu, m'}|^2
\end{equation*}
