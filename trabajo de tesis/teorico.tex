...
Si la estructura de guías de onda no varía en la dirección $z$, por separación de variables la solución para el campo eléctrico es una onda plana del tipo $\textbf{E}(\textbf{r}) = \textbf{E}_\nu(x, y) e^{i\beta_\nu z}$. A su vez, es conveniente separar el Laplaciano como $\nabla^2 \equiv \nabla_\perp^2 + \frac{\partial^2}{\partial z^2}$. De esta forma, definiendo el operador $G_{ij}$ tal que $G_{ij} E_k \equiv \partial_i\left(\frac{\partial_j n^2}{n^2}E_k \right)$, la parte transversal de las ecuaciones (\ref{eqn:helmholz}), se puede escribir como:
...
\end{align}
La ecuación (\ref{eqn:eigenfield}) es un problema de autovalores $\beta_\nu^2$ y autofunciones $\textbf{E}_\nu^\perp(x,y)$, las cuales son ortogonales en la condición de guiaje débil (ver apéndice \ref{sec:orto}). 
