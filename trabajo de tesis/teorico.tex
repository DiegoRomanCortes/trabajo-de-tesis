...
\end{align}
Para $\ell = 0$, se recuperan las relaciones obtenidas para los modos  TE y TM. El caso $\ell=1$ es de especial interés. La $m$-ésima asíntota estará determinada por el $m$-ésimo cero de $J_1(x)$. Por lo que, debido a un razonamiento similar al caso $\ell =0$, las condiciones de corte son
\begin{align*}
	(\lambda_c)_{1,m} &= 2\pi a \frac{\sqrt{n_1^2 - n_0^2}}{x_{1,m}} \quad \text{(modos HE)}
	\\
	(\lambda_c)_{1,m} &= 2\pi a \frac{\sqrt{n_1^2 - n_0^2}}{x_{1,m+1}} \quad \text{(modos EH)}
\end{align*}
