...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{media/slabgraphical.pdf \label{fig:graphTE}}
...
\end{figure}
Luego de encontrar $k_z$, se tiene todo lo necesario para reemplazar en las expresiones obtenidas para las componentes del campo electromagnético. En la Figura \ref{fig:E-slab-graph} se grafican cuatro modos del campo eléctrico.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{media/TETMfields.pdf}
	\caption[Soluciones gráficas de los modos TE]{Soluciones gráficas de los modos TE. A mayor contraste $\Delta n = n_1-n_0$, mayor cantidad de modos guiados soporta la guía de onda. \label{fig:E-slab-graph}}
\end{figure}
...
Una forma de encontrar el valor experimental del acomplamiento $V$ es mediante la excitación individual de un modo. Con esto, la razón entre las potencias $|a_1(z)|^2$ y $|a_2(z)|^2$ permite despejar el acoplamiento para una distancia de propagación $z=L$ conocida:

\begin{equation}
	V = \frac{1}{L}\tan^{-1}\left( \left|\frac{a_2(L)}{a_1(L)}\right|^2 \right)
\end{equation}
...
\subsubsection{Red de Su-Schrieffer-Heeger fotónica}
...
\centering
	...
	\caption[Espectro de la red SSH.]{Espectro de la red SSH. Condiciones de borde abiertas en función del parámetro $\delta=\frac{t}{t'}$, para $t'=1$ fijo y para un número $N=100$ de celdas unitarias. Sólo hay estados de borde a energía cero para $|\delta| < 1$.
	\label{fig:ssh-open}}
\end{figure} 
